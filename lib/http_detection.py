...
class HttpPredict():
    ...
    def predict(self, data):
            # print(data)
        anomaly_predict = self.ae.predict(data)
        # print(anomaly_predict)
        self.log(anomaly_predict)
        self.log(self.ae.threshold)
        if anomaly_predict > self.ae.threshold:
            predict_score = 1
            self.log("false postive")
        else:
            self.log("false negative")
            predict_score = 0
    ...
    def loadModelInit(self):
        ...
        basePath="."
        model_dir_path = '/opt/AutoEncoder/models'
